...
;;   New York: Norton.
;;
;; Copyright (C) 2018 Luis R. Izquierdo, Segismundo S. Izquierdo,
...
globals [
  ...
  %-unhappy-agents  ;; percentage of unhappy agents
]

turtles-own [
  happy?         ;; indicates whether at least %-similar-wanted percent
...
to do-plots-and-statistics
  set %-unhappy-agents 100 * (count turtles with [not happy?]) / (count turtles)
  let list-of-%-similar ([similar-nbrs / n-of-my-nbrs] of turtles with [n-of-my-nbrs > 0])
  set avg-%-similar 100 * mean list-of-%-similar
...
;; This model can be implemented more efficiently.
;; In particular, the following two lines of code
;; make many computations that could be saved:
;;
;; ask one-of turtles with [not happy?] [move]
;;  (we could keep a list of turtles that are not happy)
;;
;; move-to one-of patches with [not any? turtles-here]
;;  (we could have define a patch-variable turtle-here?
;;   to keep track of whether eac patch is empty or not)
@#$#@#$#@
GRAPHICS-WINDOW
219
10
...
"" ""
PENS
"percent" 1.0 0 -16777216 true "" "plot %-unhappy-agents"
